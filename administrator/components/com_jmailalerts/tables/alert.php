 ...
 * alert Table class
 */
class JmailalertsTablealert extends JTable {
	...
	public function bind($array, $ignore = '') {
		...
		if(!JFactory::getUser()->authorise('core.admin', 'com_jmailalerts.alert.'.$array['id'])){
			$actions = JFactory::getACL()->getActions('com_jmailalerts','alert');
			$default_actions = JFactory::getACL()->getAssetRules('com_jmailalerts.alert.'.$array['id'])->getData();
			$array_jaccess = array();
		...
		$array['template']=$this->getImageUrl($array['template']);//$tp_data['message_body'];
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_jmailalerts.alert.' . (int) $this->$k;
    }
    ...
    */
	...
	@param $mbody STRING which contain template data
	*
	@return a string with absolute links
	*/
	function getImageUrl($m_body)
	{
		if (!function_exists('isRelativeUrl')) {
			function isRelativeUrl($str)
			{
				//print"<pre>";print"<br>";print_r($str);
				// if string doesn't start with http
				if(	substr( $str[0], 0, 4 ) != "http")
				{
					$str[0]=JURI::root().$str[0];
					//print"------------=-----------".$str[0];
				}
		...
		$hrefpattern = "/(?<=href=(\"|'))[^\"']+(?=(\"|'))/";
		$m_body=preg_replace_callback($hrefpattern, 'isRelativeUrl', $m_body);

		// MAKE <img src absolute path
		$srcpattern = "/(?<=src=(\"|'))[^\"']+(?=(\"|'))/";
		$m_body=preg_replace_callback($srcpattern, 'isRelativeUrl', $m_body);
		return $m_body;
	}
}
