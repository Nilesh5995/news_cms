		...
		<tbody>
			<?php if (!empty($this->items)) : ?>
				<?php foreach ($this->items as $i => $row) : 
					$link = JRoute::_('index.php?option=com_school&task=student.edit&id=' . $row->id);
					//echo $row->image;
					   $row->image = new Registry;
					   //$item->imageinfo = $registry->toArray();					 //print_r($row->image);
					
					//echo $row->image->loadString($row->imageInfo);
